...
export default function RulerTimeline({
  ...
  const draw = useCallback(() => {
    ...
    // Resolve CSS vars every frame so canvas respects current theme
    const cs      = getComputedStyle(document.documentElement)
    const C_BG    = cs.getPropertyValue('--bg').trim()
    const C_BG2   = cs.getPropertyValue('--bg2').trim()
    const C_BG3   = cs.getPropertyValue('--bg3').trim()
    const C_ACCENT= cs.getPropertyValue('--accent').trim()
    const C_TEXT3 = cs.getPropertyValue('--text3').trim()
    const C_TEXT2 = cs.getPropertyValue('--text2').trim()
    const C_BORDER= cs.getPropertyValue('--border').trim()
    ...
    grd.addColorStop(0.5, `${C_ACCENT}22`)
